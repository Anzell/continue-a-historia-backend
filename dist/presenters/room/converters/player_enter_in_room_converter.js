...
class PlayerEnterInRoomConverter {
    handle(params) {
        if (params.roomId === undefined || params.roomId === "") {
            return either_ts_1.left(new failures_1.ValidationFailure({ message: PlayerEnterInRoomConverterErrorMessages.missingRoomId }));
        }
        if (params.userId === undefined || params.userId === "") {
            return either_ts_1.left(new failures_1.ValidationFailure({ message: PlayerEnterInRoomConverterErrorMessages.missingUserId }));
        }
